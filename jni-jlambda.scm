    ...
    (lambda (x r c)
      (let* ((%let*               (r 'let*))
        ...
        `(,%let* ((signature (,%type-signature ,type))
                  ...
                  (jfield  (,%get-jfield jclass ,field-name signature)))
                 (,%if jfield
                       (,%getter-with-setter (,%lambda ()
                                                       ((,%field-accessor-for get #t ,type) jclass jfield))
                                             (,%lambda (value)
                                                       ((,%field-accessor-for set #t ,type) jclass jfield value))
                                             ,field-fullname)
        ...
        `(,%let* ((signature (,%type-signature ,type))
                 ...
                 (,%if jfield
                       (,%getter-with-setter (,%lambda (object)
                                                       ((,%field-accessor-for get #f ,type) object jfield))
                                             (,%lambda (object value)
                                                       ((,%field-accessor-for set #f ,type) object jfield value))
                                             ,field-fullname)
