...
(jlambda-non-overloaded-method jlambda-method-define)
...
(define-for-syntax (%arg-types->arg-names arg-types)
  (map (lambda (type idx)
         (symbol-append (%arg-type->arg-name type)
                        (string->symbol (->string idx))))
       arg-types
  ...
  (ir-macro-transformer
   (lambda (x i c)
	...
	(let* ((%modifier (strip-syntax modifier))
               (arg-types (if (eqv? 'constructor %modifier)
                              (remove not (cons* return-type method-name arg-types))
                              arg-types))
               (method-name (and (not (eqv? 'constructor %modifier))
                                 method-name))
               (return-type (and (not (eqv? 'constructor %modifier))
                                 return-type))
               (arg-names (strip-syntax (%arg-types->arg-names arg-types)))
               (%call-variant (%call-proc-variant %modifier (strip-syntax return-type))))
	  `(let ((method (method-id ,modifier ,class-object ,return-type ,method-name ,@arg-types))
		 ...
		 (target ,class-object))
	     (extend-procedure
	      (lambda ,(jlambda-args %modifier arg-names)
