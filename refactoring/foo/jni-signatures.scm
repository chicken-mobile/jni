(module jni-signatures
*
(import chicken scheme extras data-structures)
(import-for-syntax chicken scheme extras)

(define (expand-type type . return-type)
  (let ((return-type (or (and (not (null? return-type)) (car return-type)) #f)))
    (cond ((symbol? type)
	   (case type
	     ((boolean) "Z")
	     ((byte)    "B")
